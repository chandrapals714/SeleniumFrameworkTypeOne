...
namespace Personify.SeleniumHelpers.Generators
{
    public class MersienneTwister
    {
        private const int N = 624;
        private const int M = 397;
        private const uint K = 0x9908B0DFU;
        private const uint DefaultSeed = 4357;

        private readonly ulong[] _state = new ulong[N + 1];
        private int _next = 0;
        private readonly ulong _seedValue;


        public MersienneTwister()
        {
            SeedMT(DefaultSeed);
        }
        public MersienneTwister(ulong seed)
        {
            _seedValue = seed;
            SeedMT(_seedValue);
        }
        ...
        private ulong ReloadMT()
        {
            ...
            for (s0 = _state[0], s1 = _state[1], j = N - M + 1; --j > 0; s0 = s1, s1 = p2[p2pos++])
                p0[p0pos++] = pM[pMpos++] ^ (mixBits(s0, s1) >> 1) ^ (loBit(s1) != 0 ? K : 0U);


            for (pM[0] = _state[0], pMpos = 0, j = M; --j > 0; s0 = s1, s1 = p2[p2pos++])
                p0[p0pos++] = pM[pMpos++] ^ (mixBits(s0, s1) >> 1) ^ (loBit(s1) != 0 ? K : 0U);
